...
import { blogs, countries, leads, services, settings, users } from './schema';
...
async function main() {
  ...
  const countryRows = [
    {
      ...
      content: {
        ...
        overview:
          'Canada offers internationally recognized degrees, diverse campuses, and clear post-study work pathways for international students seeking quality education abroad.',
        ...
        seo: {
          title: 'Study in Canada | Shyaam International',
          description: 'Expert Canada student visa guidance from Shyaam International.',
        },
      },
    },
    {
      ...
      content: {
        ...
        overview:
          'The UK is home to some of the worlds most prestigious universities, offering shorter degree durations and excellent graduate visa options for international students.',
        benefits: defaultBenefits,
        universities: [],
        popularCourses: [],
        ...
        faqs: [],
        seo: {
          title: 'Study in UK | Shyaam International',
          description: 'UK student visa consultancy from Shyaam International.',
        },
      },
    },
  ];
  ...
  for (const country of countryRows) {
    ...
    if (existing) {
      await db
        .update(countries)
        .set({ name: country.name, flag: country.flag, content: country.content })
        .where(eq(countries.slug, country.slug));
    } else {
  ...
  const serviceRows = [
    {
      ...
      content: {
        ...
        overview:
          'Our student visa service covers university shortlisting, documentation, application filing, and interview preparation to maximize your approval chances.',
        ...
        seo: {
          title: 'Student Visa Services | Shyaam International',
          description: 'Expert student visa consultancy services.',
        },
      },
    },
    {
      ...
      content: {
        ...
        overview:
          'We handle tourist visa applications with meticulous document preparation and embassy guideline compliance for smooth approvals.',
        ...
        faqs: [],
        seo: {
          title: 'Tourist Visa Services | Shyaam International',
          description: 'Professional tourist visa assistance.',
        },
      },
    },
  ];
  ...
  for (const service of serviceRows) {
    ...
    if (existing) {
      await db
        .update(services)
        .set({ name: service.name, content: service.content })
  ...
  const blogRows = [
    ...
    },
    {
      ...
      seo: {
        ...
        description: 'Post-study work visa options in the UK.',
      },
    },
  ];

  for (const blog of blogRows) {
    const [existing] = await db.select().from(blogs).where(eq(blogs.slug, blog.slug)).limit(1);
    if (existing) {
      await db.update(blogs).set(blog).where(eq(blogs.slug, blog.slug));
    } else {
  ...
  if (Number(leadCount?.value ?? 0) === 0) {
    for (const [index, lead] of leadRows.entries()) {
