...
export const testimonialsController = {
  list: asyncHandler(async (req: Request, res: Response) => {
    const result = await testimonialsService.list(req.query as never);
    PaginatedResponse.send(res, result.data, result.meta);
  }),

  getById: asyncHandler(async (req: Request, res: Response) => {
    const item = await testimonialsService.getById(paramString(req.params.id));
    ApiResponse.success(res, item);
  }),

  create: asyncHandler(async (req: Request, res: Response) => {
    const item = await testimonialsService.create(req.body);
    ApiResponse.success(res, item, 'Created', 201);
  }),

  update: asyncHandler(async (req: Request, res: Response) => {
    const item = await testimonialsService.update(paramString(req.params.id), req.body);
    ApiResponse.success(res, item);
  }),

  delete: asyncHandler(async (req: Request, res: Response) => {
    await testimonialsService.delete(paramString(req.params.id));
