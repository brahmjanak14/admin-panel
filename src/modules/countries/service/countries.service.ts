...
import { countriesModel, type CountryContent } from '../model/countries.model';
...
export const countriesService = {
  async list(query: { page?: number; perPage?: number; search?: string }) {
    ...
    const result = await countriesModel.findMany(query);
    const response = {
      data: result.items,
  ...
  async getBySlug(slug: string): Promise<CountryContent> {
    ...
    const country = await countriesModel.findBySlug(slug);
  ...
  async create(input: CountryContent): Promise<CountryContent> {
    if (await countriesModel.slugExists(input.slug)) {
      throw new AppError('A country with this slug already exists', 409);
    }
    const country = await countriesModel.create(input);
  ...
  async update(oldSlug: string, input: CountryContent): Promise<CountryContent> {
    ...
    if (oldSlug !== input.slug && (await countriesModel.slugExists(input.slug))) {
      throw new AppError('A country with this slug already exists', 409);
    }
    const country = await countriesModel.update(oldSlug, input);
  ...
  async delete(slug: string): Promise<void> {
    ...
    await countriesModel.delete(slug);
